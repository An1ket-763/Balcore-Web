...
> 1. Post bid at $39.95 and ask at $40.05 (spread = $0.10). 2. Buyer hits the ask → sell 10 AVAX at $40.05 → earn $0.05/AVAX on that side. 3. Seller hits the bid → buy 10 AVAX at $39.95 → earn $0.05/AVAX on that side. 4. Inventory stays neutral if both sides fill equally. 5. Continuously adjust quotes as mid-price moves → Capture spread revenue across thousands of round-trips per day.

> **Spread Revenue Calculation**
> Revenue per round-trip = (Ask − Bid) × Quantity. Annual spread revenue = avg_spread × avg_daily_volume × 365. Example: $0.10 spread × 10,000 AVAX/day × 365 = $365,000/year from one pair.
...
> 1. Pool holds: 10,000 AVAX + 400,000 USDC (k = 4,000,000,000). 2. Trader sends 100 AVAX to the pool. 3. New AVAX reserve: 10,100. 4. New USDC reserve: 4,000,000,000 / 10,100 = 396,039.60 USDC. 5. Trader receives: 400,000 − 396,039.60 = 3,960.40 USDC. 6. Effective price: 3,960.40 / 100 = $39.60/AVAX (vs $40.00 spot — slippage). → LP earns fee on this trade; pool reserves update automatically.
...
> Y_total = α × Y_LP + (1 − α) × Y_reserve − C_borrow. α = 0.10 (active LP fraction). Y_LP = fee yield from active LP position. Y_reserve = passive yield from reserve (Benqi etc.). C_borrow = cost of any outstanding borrow (if applicable). Both layers generate yield simultaneously — no capital is ever truly idle.
